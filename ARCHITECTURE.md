...
★ **알림이 자기 성적을 달고 나간다** (2026-08-06). 스크리너는 내버려 두면 자신감 기계가 된다 — 사람은 맞은 종목만 기억하기 때문이다. 그래서 낸 신호를 전부 기록하고 사후 수익률을 채워, 알림을 받는 순간 **"이걸 얼마나 믿어야 하나"**가 같이 오게 한다. 재료(`signals` 전량 기록 · `ohlcv_cache` 영구 보관)는 원래 다 있었고 계산만 없었다 (4.8).
...
| **`backtest`** | 한 종목을 하루씩 되감아 **조건을 만족한 날을 차트에 찍는다** (12.7) |
| **`serve`** | 위를 스케줄로 돌리고 알림을 보낸다 (8장) |

★ **스크리너는 내버려 두면 자신감 기계가 된다** — 사람은 맞은 종목만 기억하기 때문이다. 그걸 막는 장치는 지금 셋이고, 셋 다 "보기 싫은 것을 같이 보여 주는" 쪽으로 만들어져 있다.

| 장치 | 막는 것 |
| :--- | :--- |
| `signals` · `stats` | 내보낸 신호가 **전부** 남는다. 맞은 것만 기억하는 것을 기록이 막는다 |
| `backtest`의 배너 | 단일 종목 리플레이가 **실제 신호가 아니라는 사실**을 화면에 붙박이로 적는다 (12.7) |
| 4.8 "백테스트를 대하는 자세" | 파라미터를 뒤져 좋은 값을 찾는 것을 규범으로 막는다 |

⚠️ **`review`(신호 이력 + 이후 경로)는 만들지 않기로 했다.** `backtest`가 그 자리를 대신하지 **않는다** — 하나는 내가 고른 종목의 과거 재생이고, 다른 하나는 실제로 낸 신호의 사후 성적이다. 후자가 **진짜 out-of-sample**이고 백테스트보다 정직한데, 그걸 보는 화면이 없어졌다. 남은 창구는 `signals`·`stats`뿐이고 **사후 수익률·hit rate·IC는 아직 비어 있다** (4.8). 이 자리가 비어 있다는 것을 잊지 않기 위해 여기에 적어 둔다.
...
| **시장 중립 코어** | 파이프라인은 어느 거래소인지 모른다. 시장별 차이는 Provider와 Calendar 뒤로 숨긴다 |
| **결정성** | 같은 입력 + 같은 시각 → 같은 출력. 백테스트와 실행이 동일 코드 경로를 쓴다 |
| **시간 주입** | 파이프라인과 전략은 `datetime.now()`를 호출하지 않는다. 모든 시각은 실행 컨텍스트가 준다 |
| **격리된 실패** | 종목 하나의 실패가 실행 전체를 중단시키지 않는다 |
| **기본은 안전** | 부작용은 명시적 옵트인. 기본 모드는 `notify`, 기본 실행은 dry-run |
| **관측 가능성** | 모든 실행의 단계별 입/출력이 저장되어 사후 재현이 가능하다 |
...
```text
      사람                    serve                  LLM (Claude Code 등)
       │ backtest · explain       │ ingest --commit        │ explain --json
       │ stats · signals list     │ run --commit → 알림     │ strategy check
       ▼                          ▼                        ▼
┌──────────────────────────────────────────────────────────────┐
...
└───────────────┬──────────────────────────────────────────────┘
┌───────────────▼──────────────────────────────────────────────┐
│  Pipeline (함수 하나 — 유니버스→봉→전략→기록→로그)              │
...
| ~~DAG~~ | — | ⚠️ **2026-08-06에 걷어냈다.** NetworkX 의존성도 함께 빠졌다. 실행 경로에 **분기**가 생기면 그때 다시 판단한다 (4.3) |
| 지표 | **전략 작성자 자유** | 전략이 파이썬 클래스가 되면서 전역 결정이 아니게 됐다 |
| 시세 (일봉) | **PyKRX · yfinance · FDR** | **전부 무인증.** 일봉 고정(3.6)의 최대 이득 |
...
`backtest` 리포트가 보는 것은 반년 전 판단이다. CDN 링크가 죽으면 그 화면은 존재 이유를 잃는다. 빌드된 standalone 파일을 `app/report/vendor/`에 커밋하고 렌더 시점에 `<script>` 안으로 인라인한다 — **빌드 단계도 `package.json`도 생기지 않으므로** "Node·pnpm 툴체인 없음"이 유지된다.
...
**이 시스템의 난이도는 대부분 여기에 있고, 동시에 존재 이유도 여기에 있다.** 한국과 미국을 하나의 유니버스로 다루려면 심볼 표기, 거래 시간, 캔들 마감, 통화가 전부 달라진다.
...
class InstrumentRef:
    venue: str              # "krx" | "nasdaq" | "nyse"
    symbol: str             # "KRW-BTC" | "BTC/USDT" | "005930" | "AAPL"
    quote_currency: str     # venue가 고정한다 (krx=KRW, nasdaq/nyse=USD)
    display_name: str

    @property
    def key(self) -> str:   # "krx:005930", "nasdaq:AAPL"
        return f"{self.venue}:{self.symbol}"
```

**`quote_currency`는 venue가 고정한다** — `krx`는 KRW, `nasdaq`·`nyse`는 USD다.

> ⚠️ **2026-08-06 이전에는 그렇지 않았다.** 코인이 있던 시절에는 업비트에 KRW 말고 BTC·USDT 마켓이 있어서 통화를 심볼에서 파싱해야 했고(`VenueSpec.quote_style` — `dash_prefix` / `slash_suffix`), venue 상수로 박으면 `upbit:BTC-ETH`의 결제 통화가 KRW로 잘못 붙었다. 코인을 걷어내면서 그 계층 전체가 사라졌다. **주식만 다루는 한 되살릴 이유가 없다.**
...
**`capabilities`는 라우팅이 읽는 계약이다.** 시세 우선순위(라우팅 표)와 목록 조회 능력
(`provides_universe`)은 **별개**다 — `krx: pykrx→fdr`에서 앞의 pykrx가 목록을 못 준다는
이유로 유니버스가 통째로 실패하면 안 된다. 능력으로 고르는 것은 폴백이 아니다(항상 같은
소스가 뽑히므로 "그날의 후보 집합이 달라진다"는 문제가 생기지 않는다).

### 3.4 Connection과 라우팅 — 소스는 설정에 없다

**설계 결정**: 데이터 소스를 사람이 고르게 하지 **않는다.**

| 소스를 설정에 적게 하면 | 결과 |
| :--- | :--- |
| 파이프라인이 소스에 종속 | yfinance→Alpaca 교체 시 파이프라인을 뜯어고쳐야 함 |
| 멀티마켓 혼합이 깨짐 | 시장마다 소스를 매번 적어야 한다. Fresh Bar Gate(3.5) 무력화 |
| 폴백 불가 | 소스 하나가 죽으면 파이프라인 실패 |
| 캐시 공유 어려움 | 파이프라인마다 같은 종목을 중복 호출 |

**대신 3계층으로 나눈다.**

**① Connections** — 소스별 자격 증명. 무인증 소스(PyKRX·yfinance·FDR)는 등록 없이 항상 활성이므로, 현 범위에서 이 계층은 사실상 비어 있다.
...
**③ `source` 인자** — 기본 `"auto"`(라우팅 표). 내부 함수는 특정 소스 ID를 받을 수 있고 테스트·정합성 비교가 그것을 쓴다. **설정에는 노출하지 않는다** — 사람이 고를 이유가 없다.

이 구조에서 **새 소스 추가 = Provider 파일 1개**이며, 기존 파이프라인은 수정할 필요가 없다.

### 3.5 혼합 파이프라인 사용성 — Fresh Bar Gate

**한국과 미국을 한 파이프라인에 넣었을 때 가장 흔한 문제**: 미국장이 닫혀 있는 시간에 파이프라인이 돌면 미국 쪽은 어제와 똑같은 캔들을 다시 읽고, 같은 신호를 매번 재발생시킨다.
...
덕분에 하나의 파이프라인에 한국·미국을 함께 넣고 **매시간 실행**해도 각 시장이 장중에만 판정된다. 사용자는 캘린더를 신경 쓸 필요가 없다.

**★ 봉의 소비는 실행이 성공한 뒤에만 확정한다.** 읽는 시점에 바로 "봤다"고 기록하면, 뒤의 기록 단계가 실패했을 때도 봉이 소비된 것으로 남아 **재실행하면 stale로 걸러지고 그 신호는 영영 사라진다.** 그래서 `bar_state.stage()`로 예약만 하고, `execute()`가 `SUCCESS`일 때만 `commit()`한다.

- `partial`도 커밋하지 않는다 — 실패한 단계가 하필 기록이었을 수 있다.
...
dry-run도 **읽는다.** 읽지 않으면 `run`과 `run --commit`이 서로 다른 종목 집합을 보게 되어 dry-run이 실제 실행을 예측하지 못한다. **쓰지 않는 것과 읽지 않는 것은 다르다.** `bar_key`는 `(단계, 심볼, 타임프레임)`이라 설정이 달라도 단계 id가 같으면 충돌하므로 `pipeline_id`로 한 겹 더 나눈다.
...
- **기록하는 값은 설정이 아니라 소스가 실제로 준 것이다.** 설정을 그대로 베끼면 소스가
  조정가를 주지 않았는데도 조정가를 받은 것처럼 남는다.
...
매 실행마다 외부 API를 직접 호출하면 **스크리너로 200종목을 돌리는 순간 무료 API가 차단된다.** 수집을 실행 경로에서 분리한다.

```
[Ingestion Worker] ──주기 수집──▶ [ohlcv_cache] ◀──읽기 전용── [봉 수집 단계]
```
...
- 수집 대상은 **활성 파이프라인이 참조하는 instrument의 합집합**에서 자동 도출한다. 유니버스
  파이프라인과 **같은 함수로** 얻는다 — 컷 조건을 다시 구현하면 실행이 훑는 것과 캐시가
  담는 것이 갈라지고, 갈라진 종목은 실행 때 캐시 미스로 조용히 소스를 두드린다.
- ★ **한 번이라도 신호가 난 종목은 유니버스에서 빠져도 계속 수집한다** (규칙 18). 1월에 신호가
  났던 종목이 3월에 거래대금 상위에서 밀리면 그 시점부터 봉이 끊기고, `backtest`의 차트가 3월에
...
- **파이프라인은 캐시 구현을 모른다.** 보는 것은 "캐시를 읽는다"는 인터페이스뿐이므로 뒤를 교체할 수
...
- ★ **`lookback`(요청한 깊이)을 함께 남긴다.** 캐시가 그보다 얕으면 **소스에 더 없는
  것**이다(신규 상장). 이 값이 없으면 "아직 덜 모았다"와 "원래 이것뿐이다"를 구분할 수
  없어서 이력이 짧은 종목이 매 실행마다 소스를 다시 부른다 — 실측 360종목 중 32종목이
  그랬다. `run`도 이 이력을 남기므로 `ingest`를 쓰지 않는 사용자에게도 적용된다.
...
12개월 모멘텀의 분포는 시장마다 스케일이 다르다. 미국 성장주는 국내 대형주보다 꼬리가 훨씬
두껍고, 코스닥 소형주는 코스피 대형주보다 분산이 넓다. 이걸 한 풀에 넣고 정렬해 상위 20%를
자르면 **위쪽이 분산 넓은 쪽으로 쏠린다.** 그쪽이 더 좋아서가 아니라 **변동성이 커서**다.

> 결과적으로 모멘텀으로 줄 세운 게 아니라 **변동성으로 줄 세운 것**이 된다. 그리고 그 표는
> "종목을 골랐다"가 아니라 **"그 기간에 잘 간 시장을 골랐다"**를 뜻한다.

두 번째 피해는 `rank` · `percentile` · `universe_size`가 거짓말을 하는 것이다. `explain`이 "순위 7 / 290 (상위 2.4%)"라고 보여줘도, 삼성전자가 **미국주 섞인 290개 중 7등**이라는 문장은 해석할 수 없다.

- **시장별로 따로 랭킹하고 시장별로 자른 뒤 합집합**을 만든다. 정규화 가정이 하나도 필요 없고,
  각 시장의 대표성이 보장되며, "krx 195 중 7등"이 해석 가능한 문장이 된다.
...
### 4.3 실행 — 함수 하나 (2026-08-06)

**DAG 엔진을 걷어냈다.** 노드·그래프·레지스트리·위상정렬·on_error 정책이 있었지만
**만들어지는 그래프가 언제나 하나**였다 — 설정 파일 두 개가 똑같은 배선을 갖고 다른
것은 전략 하나였다. 조합이 하나면 조합을 표현하는 장치가 값을 못 하고, 대신 "어느
노드에서 끊겼나"를 사람이 매번 되짚어야 했다.

```
유니버스 → 봉 수집 → 전략 → 기록 → 로그
```

`app/pipeline.py`를 위에서 아래로 읽으면 그것이 프로그램의 전부다.
...
| 소스 실패 재시도 | `_with_retry` — **유니버스 조회에만** 건다. 종목 하나의 실패는 그 종목만 빠지지만(Provider 라우팅이 이미 폴백한다) 유니버스가 실패하면 그날 훑을 대상이 통째로 없어진다 |
| 외부 전송 차단 (규칙 13) | ★ **없앴다. 더 강해졌다** — 아래 |
| 분봉 차단 (규칙 12) | 타임프레임이 상수다. 설정할 자리가 없으므로 어길 수 없다 |
| DAG 검증(사이클·핸들) | 그래프가 없으므로 검증할 것도 없다 |

★ **규칙 13이 런타임 검사에서 구조로 바뀌었다.** 예전에는 노드가
`sends_external_messages = True`를 선언하고 엔진이 그걸 보고 건너뛰었다 — 선언을
빠뜨리면 새는 구조라 "노드마다 심으면 언젠가 빠뜨린다"는 경고가 필요했다.
**지금 `app/pipeline.py`에는 바깥으로 나가는 코드 경로가 아예 없다.** 알림은
`app/serve.py`가 실행 **뒤에** 보낸다. 빠뜨릴 자리가 없어졌고, 회귀 테스트가 그
파일에 전송 코드가 들어오는 것을 막는다.

**⚠️ 언제 DAG를 다시 들일 것인가** — 실행 경로에 **분기**가 생길 때다. 브랜치가
필요해지면 그때가 그래프가 값을 하는 시점이고, 그 전까지는 함수 하나가 맞다.
지금 없는 것을 미리 만들지 않는다.
...
| `signals` | ✅ | 생성된 신호 + `dedup_key` UNIQUE + 사후 수익률(`fwd_*`·`bench_*`). ⚠️ `acted`는 2026-08-07에 쓰지 않게 됐지만 **컬럼은 남긴다**(자산 테이블이라 컬럼을 못 지운다) |
...
| `instruments` | ✅ | **심볼 마스터 캐시.** 아래 참조 |
| `market_calendar` | ⬜ | 휴장일 캐시 (`exchange_calendars`가 대신하고 있다) |
| `orders` / `positions` | ⬜ P5 | |
...
**`instruments` — 심볼 마스터 캐시. ★ 거래대금은 넣지 않는다**

`ohlcv_cache`가 봉을 아끼듯 이 표는 **목록 조회**를 아낀다. 매 실행마다 FDR의 미국 목록 6,700행을 다시 받던 것이 여기서 멈춘다(64초 → 7초).

다만 목록 응답에는 **성격이 다른 둘**이 섞여 있고, 이걸 구분하는 것이 설계의 전부다.
...
- **이 표는 자산이 아니라 캐시다** — 지워도 다시 받으면 그만이라 규칙 16의 대상이 아니다.
- ⚠️ 소스 목록에 **같은 심볼이 두 번** 나온다(FDR 나스닥). 먼저 나온 것을 남긴다 —
  목록 순서가 곧 중요도라 앞엣것이 대표 표기다.

⚠️ **`create_all`은 기존 테이블에 컬럼을 더하지 않는다.** 모델에 컬럼을 추가하면 예전 DB는 그대로 남고, 그 DB에 쓰는 순간 캐시 경로가 예외를 삼켜 **"조금 느린 것"처럼 보인다** — 실제로는 캐시가 영영 안 채워진다. 실제로 밟은 사고라 `init_db()`가 컬럼 누락을 확인해 `SchemaDriftError`로 앞당겨 터뜨린다. Alembic 전까지의 임시방편이지만 조용한 열화보다 낫다.
...
| ★ **채점 대상** | **낸 신호 전부.** 사용자가 실제로 무엇을 샀는지는 묻지 않는다. 아래 참조 |

**★ 신호는 전부 샀다고 가정한다** (2026-08-07)

성적표는 그 구간의 신호를 **전량 매수했다고 치고** 사후 수익률을 낸다. 사용자가 무엇을 실제로 샀는지는 시스템이 알지 못하고, **알려고 하지 않는다.**

> ⚠️ **가정을 리포트에 매번 적는다.** 안 적으면 사람은 이 숫자를 **자기 계좌 수익률**로 읽는데, 실제로는 산 적 없는 종목까지 들어 있는 값이다. 그 오독이 정확히 1.2가 막으려는 "자신감 기계"다. `tests/test_scorecard.py`가 그 문구를 고정한다.

**⚠️ 오버라이드 추적은 2026-08-07에 걷어냈다. 되살리지 않는다.**

한때 `signals.acted`에 `[샀다/안 샀다]` 응답을 받아 **산 것과 무시한 것을 갈라** 비교했다. "무시한 신호가 더 좋았다면 재량이 손해다"를 재는, 개인 투자자가 거의 보지 못하는 숫자였고 이 문서가 "이 시스템에서 가장 확실한 가치"라고 불렀다. 지운 이유는 둘이다.

1. **재료가 구조적으로 안 쌓였다.** 버튼은 텔레그램 메시지 단위라 신호 1건인 알림에만 붙일 수 있었는데(여러 건이면 어느 신호의 답인지 정해지지 않는다), 실제 신호는 하루 3~6건이라 거의 안 붙었다. 첫 성적표가 `산 것 0 · 무시한 것 0 · 미응답 55`였다.
2. ★ **더 중요한 쪽 — 절반만 쓰면 없느니만 못한 장치였다.** 이 비교는 응답을 **빠짐없이** 해야만 성립한다. 산 종목은 계좌에 있으니 답하고 무시한 종목은 넘기게 되는데, 그러면 acted/ignored가 무작위 분할이 아니라 **자기가 고른 분할**이 되어 결론이 아첨하는 쪽으로 기운다. 4.8의 나머지가 "숫자를 혼자 두지 않는다"고 정한 것과 정면으로 충돌한다.

되살리려면 **응답을 빠짐없이 받을 방법이 먼저** 있어야 한다. 없이 되살리면 편향된 분할로 "재량에 값이 있다/없다"를 판정하게 된다. `signals.acted` **컬럼은 지우지 않았다** — `signals`는 자산이고 `create_all`이 기존 컬럼을 못 지운다(4.7).
...
│   ├── serve.py      상주 루프 — 발화·알림·하트비트·월간 성적표 (8장)
...
**★ 발화 조건은 "지금이 슬롯을 지났는가"가 아니라 "직전 판단과 지금 사이를 지났는가"다.**

전자로 두면 **저녁에 서버를 켠 순간 그날의 슬롯이 전부 한꺼번에 발화한다.** 각각이 `--commit`이라 봉을 몰아서 소비하고, 그건 되돌릴 수 없다(3.5 / 규칙 11). 그래서 켤 때 이미 지난 슬롯은 **건너뛰고 그 사실을 화면에 남긴다** — 조용히 건너뛰면 "왜 안 돌았지"가 된다.

**서머타임은 날마다 다시 계산해 넘긴다.** 하루를 24시간으로 더하면 전환 날 한 시간 어긋나고, 그 어긋남이 **마감 전에 돌아 어제 봉으로 판정하는** 사고가 된다.

**★ 주말에는 발화도 하트비트도 하지 않는다** (2026-08-07). **설정 항목이 아니라 기본값이다** — 장이 서지 않는 날을 켜고 끌 이유가 없다. 주말 실행은 Fresh Bar Gate에 걸려 어차피 0건이라 신호 알림은 원래 안 나갔지만, **하트비트는 그대로 나가 이틀에 한 번꼴로 "볼 것 없는 알림"이 됐다.** 그걸 흘려보기 시작하면 정작 봐야 할 평일 알림도 같이 흘려보게 된다 — 주의력 기계가 무너지는 경로가 12.2의 "손으로 돌릴 때마다 알림이 나가는 것"과 같다.

> **판정은 로컬이 아니라 UTC 요일로 한다.** 설정의 `timezone`(예: `Asia/Seoul`) 요일로 자르면 한국에서 가장 흔한 슬롯인 **"토요일 06:10"** — 미국장 **금요일** 마감 직후 — 이 통째로 사라진다. "주말엔 조용하다"가 아니라 **"주중 하루치를 잃는다"**가 되는 것이다. UTC가 자연스러운 자인 이유는 **세계 시장의 한 주가 UTC 월~금에 담기기** 때문이다(KRX 금요일장 = 금 00:00\~06:30 UTC, 미국 금요일장 = 금 13:30\~21:00 UTC).

**⚠️ 성적표 날이 주말이면 다음 발송일로 민다.** 성적표는 하트비트에 얹혀 나가므로 `scorecard_day == 오늘`로 두면 **그 달치가 통째로 사라진다.** 한 달에 한 번뿐이라 한 번 놓치면 그만큼이 그냥 없어지고, 그건 이 프로젝트의 제품(4.8)을 잃는 것이다.
...
| # | 항목 | 왜 이 순서인가 |
| :--- | :--- | :--- |
| 1 | ~~텔레그램 `[샀다/안 샀다]` 버튼~~ | **2026-08-07에 걷어냈다.** 응답을 빠짐없이 해야만 성립하는 비교였고, 실제로는 산 것만 답하게 되어 편향됐다 (4.8) |
...
| **10** | **`acted` 응답을 어떻게 받을 것인가** | ~~텔레그램 인라인 버튼 (2026-08-06)~~ → **받지 않는다 (2026-08-07).** 버튼은 신호 1건짜리 알림에만 붙일 수 있어 하루 3~6건인 실제 신호에는 거의 안 붙었고, 무엇보다 **응답을 빠짐없이 해야만 성립하는 비교**였다 — 산 것만 답하면 자기가 고른 분할이 되어 결론이 아첨하는 쪽으로 기운다. 성적표는 신호를 **전부 샀다고 가정**한다 (4.8). **알림은 다시 나가는 방향뿐이다** |
...
| `backtest <종목> --start D [--end D]` | 없음 | ★ **그날 이 종목은 어땠나** — 날짜별 리플레이 + 차트 (12.7) |
| `serve` | 스케줄 실행마다 `--commit` · **알림 발송** | 스케줄 + 하트비트(8장). **알림이 나가는 유일한 명령.** 화면 없음 |
| `ingest [--venue V] [--commit]` | **`--commit` 시에만** | 일봉 수집 → `ohlcv_cache`. **기본은 계획만** (3.9) |
| `explain <signal_id>` | 없음 | ★ **왜 이 신호가 났는가** (12.5) |
| `signals list` | 없음 | 기록된 신호를 최신순으로 |
| `evaluate` | `signals` 갱신 | 사후 수익률을 채운다 (4.8). **외부 호출 없음** — 캐시만 읽는다 |
| `scorecard [--send]` | `--send` 시 발송 | ★ **성적표** — 신호를 전부 샀다고 치고 승률·기저율·초과수익 (4.8) |
| `stats` | 없음 | 신호 건수·분산. 사후 성적은 `scorecard`가 낸다 |
...
- **미래 차단은 데이터 쪽에서 한다.** 전략에게 넘기는 프레임은 `frame.iloc[:i+1]` — 그날 마감된 봉까지다. 뒤 구간이 아예 없으므로 `shift(-1)`도 `bfill`도 볼 것이 없다. 규칙 3의 AST 검사와 **둘 다** 통과해야 신뢰할 수 있다.
- **`compute` → `rank` → `select` 순서는 `strategies/stages.py`가 단일 출처다.** `run`의 Strategy Runner와 **같은 함수**를 부른다. 각자 순서를 적으면 언젠가 한쪽만 바뀌고, 그날부터 백테스트가 실거래와 다른 코드를 돌면서 같은 것을 재현했다고 말한다.
- **`ctx.now`는 그날로 고정된다** (규칙 1). 전략은 자기가 과거를 돌고 있다는 사실을 알 수 없다.
- 차트는 vendoring한 `lightweight-charts`로 그리고 **인라인한다**(2.1). `app/report/vendor/`에 라이선스와 함께 동봉한다. **CDN을 걸지 않는다** — 반년 뒤 링크가 죽으면 존재 이유가 사라진다.
- **봉과 마커는 `session_date()`라는 같은 함수로 날짜를 정한다.** `bar_time`은 세션 **마감** 시각이라(규칙 15) 그대로 날짜를 떼면 미국 봉이 하루 밀리고(8/3 세션 마감이 KST 8/4 새벽), 마커가 엉뚱한 봉 위에 앉는다.
- **부작용이 없다.** `signals`를 남기지 않고 봉도 소비하지 않는다 — 모드가 `backtest`라 `persists_signals`·`sends_alerts`가 구조적으로 False다(4.2). 받아 온 봉을 `ohlcv_cache`에 넣는 것은 예외인데, 그건 판단이 아니라 자료 축적이다(규칙 11).

**⚠️ 이 명령이 답하지 못하는 것 — 횡단면 컷**

종목 하나만 리플레이하면 `select`의 "상위 N개" 후보가 1개라 **항상 통과한다.** 그래서 마커는 **"조건을 만족한 날"**이지 "그날 실제로 뽑혔을 날"이 아니다 — 실제 실행에서는 같은 날 조건을 통과한 다른 종목에 밀렸을 수 있다. 두 값은 **그날 그 시장의 통과 종목이 컷보다 적었을 때만** 같다.

이 사실은 결과의 `cut_applied=False`로 실려 **리포트 상단 배너와 CLI 출력 양쪽에 그대로 적힌다.** 순위·백분위·유니버스 크기는 표에서 아예 뺀다 — 유니버스가 1이라 언제나 "1 / 1 · 상위 100%"가 되고, 그건 정보가 아니라 오해다.

> 과거 유니버스를 되살려 컷까지 재현하려면 **그날 상장돼 있던 종목 목록**이 필요한데, 거래소는 언제나 "지금"의 목록만 준다. 규칙 14가 동적 유니버스를 백테스트에서 거부하는 이유가 이것이고, 그래서 이 한계는 우회하는 것이 아니라 **적어 두는** 것으로 다룬다. 비교할 종목을 사용자가 직접 주는 `--universe`는 나중에 붙일 수 있다.

**⚠️ 없앤 것 — `review`**

"쌓인 신호 전체의 결과"를 보는 화면은 만들지 않기로 했다. `backtest`가 그 자리를 대신하지는 **않는다**: 하나는 내가 고른 종목의 과거 재생이고, 다른 하나는 실제로 낸 신호의 사후 성적이다. 후자를 보는 창구는 `signals`·`stats`이고, 사후 수익률·hit rate·IC는 아직 비어 있다(4.8). 스크리너가 자신감 기계가 되지 않게 하는 장치가 그만큼 얇아졌다는 뜻이라, **`stats`의 신호 품질 지표는 남은 숙제로 남는다.**

### 12.9 `serve` — 화면 없는 상주 프로세스 ★

**2026-08-06에 웹 UI를 걷어냈다.** 2026-08-04에 "서버 렌더 화면은 React 캔버스와 다르다"며 되살렸던 것인데, 이틀을 써 보고 접었다. 이유는 그 구분이 틀려서가 아니라 **화면이 존재한다는 사실 자체**에 있었다.
...
- **주말(UTC 토·일)에는 셋 다 쉰다** (8장). 설정 항목이 아니라 기본값이고, 토요일 새벽의 미국장 슬롯은 UTC로 금요일이라 그대로 돈다.
...
| 16 | ~~**오버라이드 추적**~~ | (없음) | **걷어냄 (2026-08-07)** | 응답을 빠짐없이 해야만 성립하는데 실제로는 산 것만 답하게 되어 편향됐다. 성적표는 신호를 전부 샀다고 가정한다 (4.8) |
| 17 | **인터페이스** | 웹 UI (React SPA + REST) | **CLI (Typer)** | 하루 3회 도는 배치에 상주 서버가 필요 없다. FastAPI·APScheduler·SSE가 함께 소멸 (12장) |
| 18 | **시각화** | 서버가 렌더한 대시보드 | **정적 HTML + vendoring한 차트** | 파일로 떨어뜨리면 반년 뒤에도 열린다. 2026-08-04에 `serve`가 그 파일을 띄우는 UI를 얻었지만, **리포트 자체는 서버 없이 열리는 성질을 유지한다** (2.1 / 12.7) |
