...
| 인터페이스 | **CLI (Typer) + 정적 HTML.** 웹 서버·캔버스 없음 |
| 자동 실행 | **`serve` 확정** — 스케줄·알림·재시도를 한곳에. 상주 프로세스를 되살린다 (8장) |
...
**정해둔 전략으로 암호화폐·한국주식·미국주식을 훑어 볼 만한 후보를 리스트업하는 개인용 CLI.** 전략은 파이썬 클래스로 쓰고, 파이프라인은 그 전략을 데이터·LLM·알림과 엮는 얇은 배선이다.

명령은 셋이다.

| 명령 | 하는 일 |
| :--- | :--- |
| **`run`** | 오늘의 후보를 뽑아 stdout과 리포트로 낸다. `--commit`이면 `signals`에 남긴다 |
| **`review`** | 남긴 신호를 모아 **"그래서 어떻게 됐나"를 차트로 본다** (12.7) |
| **`serve`** | 위를 스케줄로 돌리고 알림을 보낸다 (8장) |

★ **`review`가 이 프로젝트를 정직하게 유지한다.** 스크리너를 리뷰 없이 오래 쓰면 반드시 자신감 기계가 된다 — 사람은 맞은 종목만 기억하기 때문이다. 지나간 신호를 전부 늘어놓고 **오른 것과 내린 것을 같이 보게 강제하는 화면**이 "후보"라는 말의 무게를 지킨다.

같은 이유로 `review`는 **백테스트보다 정직한 검증**이다. 백테스트는 과거를 되감으니 미래 참조·서바이버십·과적합을 계속 의심해야 하지만, 그때 실제로 내보낸 신호를 나중에 보는 것은 **진짜 out-of-sample**이다. 그래서 백테스트는 "구현이 안 틀렸나"를 재는 도구로만 남는다 (4.8).

**이 시스템은 예측 기계가 아니라 주의력 기계다.** 시장을 맞히는 것이 아니라, 혼자서는 볼 수 없는 범위를 대신 보고 정해둔 규칙을 대신 지키는 것이 목적이다. 최종 판단은 사람이 한다. **이 문서의 거의 모든 결정이 이 한 줄에서 파생된다.**
...
```text
      사람                    serve                  LLM (Claude Code 등)
       │ review · explain         │ ingest --commit        │ explain --json
       │ stats · signals ack      │ run --commit → 알림     │ strategy check
       ▼                          ▼                        ▼
┌──────────────────────────────────────────────────────────────┐
│  marketscan CLI (Typer) — 웹 서버 없음                        │
│  run · review · serve · ingest · explain · signals ·         │
│  stats · strategy · describe · (backtest: P3.5)   (12장)     │
└───────────────┬──────────────────────────────────────────────┘
┌───────────────▼──────────────────────────────────────────────┐
...
| 리포트 | **정적 HTML 파일 생성** | 서빙하지 않고 `reports/`에 떨어뜨린다. 반년 뒤에 열어도 그대로 보여야 하므로 **외부 리소스를 참조하지 않는다** |
| 차트 | **`lightweight-charts` (Apache-2.0) — vendoring** | `review`에만 붙는다. CDN이 아니라 저장소에 박는다. 아래 참조 |
...
**★ 차트 라이브러리는 vendoring한다 — CDN을 걸지 않는다.**

`review`가 보는 것은 반년 전 신호다. CDN 링크가 죽으면 그 화면은 존재 이유를 잃는다. 빌드된 standalone 파일을 `app/report/vendor/`에 커밋하고 렌더 시점에 `<script>` 안으로 인라인한다 — **빌드 단계도 `package.json`도 생기지 않으므로** "Node·pnpm 툴체인 없음"이 유지된다.

- **왜 uPlot이 아니라 `lightweight-charts`인가** — 이 화면의 핵심이 "신호가 난 지점을 찍고 그
  이후 경로를 보는 것"인데, **마커가 1급 기능**이고 거래일 갭 처리(주말·휴장일에 빈 칸이 생기지
  않음)와 십자선 툴팁이 따라온다. 둘 다 금융 차트의 도메인 문제라 직접 만들면 길다.
  gzip 기준 크기는 두 라이브러리가 사실상 같다.
- ⚠️ **버전에 따라 시리즈 생성 API가 다르다** (v4의 `addLineSeries` ↔ v5의 `addSeries(LineSeries, …)`,
  `setMarkers` ↔ `createSeriesMarkers`). 박아 둔 버전을 `vendor/` 옆에 적어 둔다.
- ⚠️ **일봉의 `time`은 business day 문자열로 넘긴다.** 이 저장소의 `bar_time`은 세션 마감
  시각(UTC)이라(규칙 15) 그대로 타임스탬프로 주면 축이 시각 단위로 읽히고, 시장이 다른 종목을
  나란히 놓으면 하루씩 어긋나 보인다. 변환은 "UTC 마감 시각 → 그 시장의 현지 세션 날짜"로,
  `daily_frame.py`가 하는 변환의 역방향이다.
- Apache-2.0이므로 `LICENSE`를 함께 커밋하고 **attribution 로고를 끄지 않는다.**

> polars 전환은 **측정 전에 하지 않는다.** pykrx·yfinance가 pandas를 돌려주고 `Item.ohlcv`
> 계약도 pandas다. 피처 행렬이 실제로 느리면 그 계산 구간에만 넣는다.
...
- ★ **한 번이라도 신호가 난 종목은 유니버스에서 빠져도 계속 수집한다** (규칙 18). 1월에 신호가
  났던 종목이 3월에 거래대금 상위에서 밀리면 그 시점부터 봉이 끊기고, `review`의 차트가 3월에
  멈춘다. 그리고 **하필 유니버스에서 밀린 종목이 대개 내린 종목이다** — 이 누락은 리뷰 화면을
  조용히 낙관 편향시킨다. 서바이버십이 데이터 레이어로 들어오는 것과 같은 사고다(4.8).
  수집 대상에 `signals` 테이블의 종목을 합집합으로 더한다.
...
- ★ **dry-run도 캐시에 쓴다.** 규칙 11이 막는 것은 **되돌릴 수 없는 것** 셋이다 — 알림 발송,
  ...
  **자료 축적**이고, `ingest`가 어차피 쓸 것을 미리 쓰는 것뿐이다. 받아 놓고 버리면 무료
  API를 두 번 두드리게 되는데 **그 호출을 아끼는 것이 이 절의 존재 이유다.**
  `run`(dry-run)과 `explain`(읽기 전용)은 다르다 — 앞은 "부작용 없이 실행해 본다"이고
  뒤는 "조회만 한다"다. 12.1의 "DB 파일조차 만들지 않는다"는 뒤쪽의 규약이고 그대로다.
- ⚠️ **가짜 시세는 캐시에 넣지 않는다** (`capabilities.cacheable`). 캐시는 소스를 구분해
  **읽지** 않으므로(`source_id`는 기록용이다) 한 번 섞인 합성 봉이 이후 실제 실행에 그대로
  쓰이고, 삭제 경로가 없어(규칙 16) 되돌릴 수도 없다. dry-run도 쓰게 되면서 이 경로가
  실제로 밟히기 쉬워졌다.
- 백테스트 리플레이는 `writable=False`로 읽기만 한다 — 과거 시각으로 도는 실행이 캐시를
  덮으면 안 된다.
...
**★ 횡단면은 "비교 가능한 집단" 안에서만 성립한다 (규칙 17)**

12개월 모멘텀의 분포는 시장마다 스케일이 다르다 — 코인은 대략 −60%~+300%, 국내 대형주는 −30%~+60%다. 이걸 한 풀에 넣고 정렬해 상위 20%를 자르면 **위쪽이 거의 전부 코인이 된다.** 코인이 더 좋아서가 아니라 **분산이 몇 배 넓어서**다.

> 결과적으로 "멀티마켓 스크리너"라고 적어 놓고 **코인 포트폴리오에 주식 몇 개 얹힌 것**이 나온다.
> 모멘텀으로 줄 세운 게 아니라 **변동성으로 줄 세운 것**이 된다.

두 번째 피해는 `rank` · `percentile` · `universe_size`가 거짓말을 하는 것이다. `explain`이 "순위 7 / 360 (상위 1.9%)"라고 보여줘도, 삼성전자가 **코인 섞인 360마리 중 7등**이라는 문장은 해석할 수 없다.

- **시장별로 따로 랭킹하고 시장별로 자른 뒤 합집합**을 만든다. 정규화 가정이 하나도 필요 없고,
  각 시장의 대표성이 보장되며, "krx 200 중 7등"이 해석 가능한 문장이 된다.
- **유동성 컷을 venue별로 나눈 것과 같은 논리다**(3.7). 원과 달러를 못 섞는다면 원화 모멘텀과
  달러 모멘텀도 못 섞는다.
- 대안인 "시장 내 z-score 후 통합 랭킹"은 **코인의 +2σ와 KRX의 +2σ가 같은 의미인가**라는 가정을
  새로 들여온다. **가정이 적은 쪽을 택한다.**
- ⚠️ **그룹 키는 `venue`가 아니라 시장이다.** `nasdaq`과 `nyse`는 같은 통화·같은 캘린더·같은
  분포라 나눌 이유가 없다. `pipeline_file.MARKETS`(crypto / krx / us)가 이미 그 그룹을 갖고 있다.
...
| | Forward Return Evaluator | ⬜ P3 | `signals` 기록 후 N봉 뒤 수익률을 소급 채운다. **`review`의 원자료**이자 4.8 신호 품질 지표의 산출원. 캐시가 있어 외부 호출이 필요 없다 |
...
- **원소가 문자열이 아니라 조건 묶음**인 이유는 venue마다 필요한 컷이 다르기 때문이다 —
  코인은 KRW 마켓 제한이 필요하고 주식은 아니며, 미국 목록에는 거래대금이 아예 없다.
...
**`run_report`에는 차트를 붙이지 않는다.** 실행 1회 리포트는 "방금 뭐가 나왔나"라 표로 충분하고, 여기까지 라이브러리를 얹으면 dry-run 스무 번에 `latest.html`이 계속 무거워진다. **차트는 `review`만.**
...
- **`serve`는 이 명령들을 그대로 부른다.** 손으로 부르든 `serve`가 부르든 동작이 같아야 한다 —
  그래서 이 결정이 늦어도 코드가 늘지 않았다.

**OS 스케줄러 대신 `serve`를 고른 이유**는 알림 때문이다. 어느 쪽을 택하든 전송 주체는 필요한데, OS 스케줄러로 가면 "알림만 담당하는 얇은 명령"이 따로 생기고 재시도·백오프가 OS 설정으로 흩어진다. 윈도우 작업 스케줄러의 마찰까지 더하면 한곳에 모으는 쪽이 값이 있다.

**⚠️ 대신 상주 프로세스가 돌아왔다. 값을 하나 치러야 한다.**

APScheduler를 기각한 이유("프로세스가 죽으면 스케줄도 같이 죽는다")가 `serve`에 그대로 적용된다. 조용히 죽으면 알림이 안 오는데, **알림이 안 오는 것과 신호가 0건인 것이 구분되지 않는다.**

> **`serve`는 신호가 0건이어도 하루 1회 하트비트를 보낸다.** 이게 없으면 어느 날부터 시스템이
> 죽어 있었는지 알 수 없다. 12.3이 "신호 0건과 실패를 구분한다"고 정한 것과 같은 문제가
> 프로세스 수준에서 반복되는 것이다.
...
| **3** | ★ **리뷰 & 상주 실행** — 신호별 사후 수익률, `review` 정적 HTML + 차트, `serve` + 알림 + 하트비트 | ⬜ |
| **3.5** | 백테스트 — 캘린더 리플레이, **피처 행렬 사전 계산**, 엔진 검증 4종 | ⬜ |
...
- ★ **리뷰가 백테스트보다 앞이다.** `review`는 그때 실제로 내보낸 신호를 나중에 보는 것이라
  **진짜 out-of-sample**이고, 백테스트는 아무리 잘 만들어도 과거를 되감는 일이라 의심할 것이
  계속 남는다. 정직한 검증을 먼저 세운 뒤 백테스트를 **구현 검증 도구로만** 붙인다(1.1 / 4.8).
  캐시(Phase 2)가 붙어서 사후 수익률 채우기에 외부 호출이 필요 없다는 점도 지금이 싼 이유다.
...
11. **`review`가 보여 줄 "그래서 어떻게 됐나"의 기준 봉 수** — 신호 이후 1·5·20봉을 함께 낼지,
   하나만 고를지. 4.8은 복수 산출을 적어 뒀으나 화면에서 무엇을 기본으로 보여 줄지는 미정이다.
   **하나만 골라 튜닝하기 시작하면 그게 파라미터 탐색이므로**(1.3) 복수 고정을 권한다.
12. **페이퍼 포트폴리오를 둘 것인가** — `review`는 신호별 사후 수익률을 보여 주지만 "이걸 따랐으면
   내 계좌가 어땠나"는 답하지 않는다. 답하려면 기계적 청산 규칙 하나("다음 리밸런스에 상위에서
   빠지면 교체")가 필요한데, 이건 전략이 아니라 **계측기**다. ⚠️ 넣는다면 **그 규칙을 절대
   튜닝하지 않는다** — 보유 기간을 바꿔 가며 제일 좋은 걸 고르는 순간 1.3이 진짜로 깨진다.
...
| — | `review` 화면의 형태 | **정적 HTML 확정.** 읽기 전용에 상호작용이 거의 없어 서버가 필요 없다. 필터는 CLI 인자로 받는다. `frontend/`를 되살리지 않는다 (12.7) |
...
| `review [--since D] [--strategy S]` | 없음 | ★ **그래서 어떻게 됐나** — 신호 이력 + 차트 (12.7) |
| `serve` | 실행마다 `--commit` · **알림 발송** | ★ 스케줄 + 알림 + 하트비트 (8장). **알림이 나가는 유일한 명령** |
...
| `backtest` / `verify` | `backtest_runs` 기록 / 없음 | **Phase 3.5.** 리플레이 + 다중검정 카운터 / 엔진 검증 4종 |
...
### 12.7 `review` — 그래서 어떻게 됐나 ★

`explain`이 **한 신호의 근거**를 답한다면, `review`는 **쌓인 신호 전체의 결과**를 답한다. 1.1이 말한 "리뷰 없는 스크리너는 자신감 기계가 된다"를 막는 화면이다.
...
- **정적 HTML 한 장이다. 서버가 아니다.** 하는 일이 "지난 신호를 늘어놓고 이후 주가를 본다"라
  읽기 전용이고 상호작용이 거의 없다. 필터는 CLI 인자로 받으므로 서버가 낄 자리가 없다.
  서버가 필요해지는 순간은 화면에서 뭔가를 바꾸면 다시 계산해야 할 때인데, 여기엔 그게 없다.
  **`frontend/`를 되살리지 않는다** (부록 C 19번).
- 차트는 vendoring한 `lightweight-charts`로 그리고, **신호 시점에 마커를 찍는다**(2.1).
- `--commit` 실행의 신호만 싣는다. dry-run은 애초에 아무것도 남기지 않으므로(12.2) 전략을
  고치며 스무 번 돌린 실험이 리뷰를 오염시키지 않는다.
- **데이터를 미리 자른다.** 신호 50건 × 전체 이력을 통째로 넣으면 한 장이 1MB를 넘는다.
  신호 전후 창(예: 앞 60봉·뒤 120봉)만 싣고, 목록의 스파크라인에는 종가만 넘긴다.
  차트 인스턴스도 50개를 한 번에 만들지 않고 **목록 클릭 → 큰 차트 하나 갱신** 구조로 둔다.
- ⚠️ **`review`는 수익률 보고서가 아니다.** 여기 나오는 것은 **신호별 사후 수익률**이고,
  "이걸 따랐으면 내 계좌가 어땠나"는 청산 규칙이 있어야 답할 수 있다(1.3 / 11장 12번).
  둘을 섞어 읽으면 없는 성과를 본 것이 된다.
...
| 18 | **시각화** | 서버가 렌더한 대시보드 | **정적 HTML + vendoring한 차트** | 서빙하지 않고 파일로 떨어뜨리면 반년 뒤에도 열린다. `review`가 읽기 전용이라 서버가 낄 자리가 없다 (2.1 / 12.7) |
| 19 | **스케줄러** | APScheduler | **`serve` 확정** | 알림 전송 주체가 어차피 필요하고, 흩어 놓는 것보다 한곳에 모으는 값이 컸다. ⚠️ 상주 프로세스가 돌아온 대가로 **하트비트가 필수**다 (8장 / 11장 4b) |
