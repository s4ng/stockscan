...
| 레이어 | 선택 | 비고 |
| :--- | :--- | :--- |
| **인터페이스** | **CLI (Typer) + `serve`의 서버 렌더 UI** | 빌드 단계 없는 Jinja 템플릿. React 캔버스는 되살리지 않는다 (12.9) |
...
│   ├── web/          app.py(FastAPI 라우트) · templates/ — 서버 렌더 UI (12.9)
...
**보안** — 웹 UI를 여는 포트 하나를 빼면 네트워크에 아무것도 열지 않는다. 남은 비밀은 **텔레그램 토큰과 (API를 쓴다면) LLM 키뿐**이며 시세에는 자격 증명이 필요 없다(3.3).
...
| **3** | ★ **백테스트 & 상주 실행** — `backtest`(날짜별 리플레이 + 차트), `serve`(웹 UI + 스케줄 + 알림 + 하트비트) | ✅ |
...
| — | `serve`에 웹 UI를 둘 것인가 | **둔다 (2026-08-04).** 서버 렌더 + 빌드 단계 없음. 실행은 전부 `app/service.py`를 지나 CLI와 같은 경로를 탄다 (12.9) |
...
| `serve` | 스케줄 실행마다 `--commit` · **알림 발송** | ★ 웹 UI(12.9) + 스케줄 + 하트비트(8장). **알림이 나가는 유일한 명령** |
...
- **봉과 마커는 `session_date()`라는 같은 함수로 날짜를 정한다.** `bar_time`은 세션 **마감** 시각이라(규칙 15) 그대로 날짜를 떼면 업비트 봉이 하루 밀리고, 마커가 엉뚱한 봉 위에 앉는다.
...
**2026-08-04에 v0.5 전환을 부분 번복했다.** 걷어냈던 것은 *파이프라인을 캔버스로 편집하는 React 앱*이고, 되살린 것은 **이미 있는 명령을 부르고 결과를 띄우는 서버 렌더 화면**이다. 그 차이가 유지되지 않으면 v0.4로 돌아간다.
...
| 화면 | 하는 일 |
| :--- | :--- |
| 대시보드 | 설정·전략(해시)·유니버스·마지막 실행 + **실행 버튼**(run dry/commit · backtest · ingest) |
| 리포트 | `~/.marketscan/reports/*.html` 목록과 뷰어 — 파일을 그대로 띄운다 |

- ★ **실행은 `app/service.py`를 통해서만 한다.** 화면이 엔진을 직접 부르면 "버튼으로 돌린 것과 터미널로 돌린 것이 다른 일을 하는" 상태가 되고, 그건 규칙 11·13을 화면 쪽으로 우회하는 것이다. 서비스 계층이 있는 이유가 이것 하나다.
- ★ **읽기는 GET, 실행은 POST.** 부작용을 GET에 두면 새로고침·프리페치가 실행을 부른다 — `--commit`이 열린 화면에서 그건 봉을 소리 없이 소비하는 길이다.
- ★ **리포트 경로를 가둔다.** 이 프로세스는 `~/.marketscan` 전체를 읽을 수 있고 거기엔 DB가 있다. 파일명에 구분자·`..`가 있으면 거부하고, 다 편 경로가 리포트 디렉터리 **안**인지 확인한다.
- ★ **화면에서 누른 실행은 알림을 보내지 않는다** (`allow_alerts=False`). 사람이 부른 실행에서 채널로 나가면 알림 자체를 믿지 않게 된다(12.2). 알림은 스케줄 경로만 연다.
- **실행은 한 번에 하나만**(`service.run_lock`). 터미널에서는 사람이 한 번에 하나씩 치니 문제가 없었는데, **버튼이 생기면 연타가 기본**이고 겹친 `--commit` 둘은 같은 봉을 두 번 소비한다.
- **기본 바인딩은 `127.0.0.1`.** 인증이 없는 화면이 `--commit`을 부를 수 있으므로, 외부에 여는 것은 명시적 옵션 + 경고로만 둔다.
- **Node·pnpm·빌드 단계는 없다.** 템플릿은 Jinja, 차트는 이미 vendoring한 lightweight-charts를 쓴다 — `package.json`이 생기는 순간 부록 C 19번이 진짜로 깨진다.

**스케줄 상태도 화면에 있다** — 다음 발화, 다음 하트비트, 최근 발화 결과, 최근 알림. **"다음 언제"와 "마지막 결과"가 같이 보여야** 죽은 것을 알아챈다.
