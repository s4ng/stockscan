...
⚠️ **그런데 지금 그 재료가 안 쌓이고 있다** (2026-08-06 관찰). `[샀다/안 샀다]` 버튼은
**신호가 1건인 알림에만** 붙는다 — 버튼은 텔레그램 메시지 단위라 여러 건을 한 메시지로
보내면 어느 신호에 대한 답인지 정해지지 않고, 그 상태로 하나를 골라 기록하면 **사용자가
누른 것과 다른 신호에 답이 붙어** 위 표의 비교가 통째로 거짓말이 된다 (`alerts.ack_buttons`,
11장 10번). 틀린 데이터가 빈 데이터보다 나쁘므로 이 차단은 옳다.

문제는 **실제 신호가 하루 3~6건씩 나온다**는 것이다. 그래서 버튼이 거의 안 붙고, 첫
성적표가 `내가 산 것 0 · 무시한 것 0 · 미응답 55`로 나왔다. 위의 "가장 확실한 가치"는
현재 **구조적으로 도달 불가능**하다. 선택지는 11장 13번에 열어 둔다.
...
13. ★ **신호 알림을 1건당 메시지 1개로 쪼갤 것인가** (2026-08-06에 열림) — 10번에서
   버튼을 "신호 1건일 때만"으로 막았는데, **실제 신호가 하루 3~6건씩 나오면서 버튼이
   거의 안 붙는다.** 첫 성적표가 `산 것 0 · 무시한 것 0 · 미응답 55`로 나왔고,
   4.8이 "이 시스템에서 가장 확실한 가치"라고 부른 오버라이드 비교가 **재료 부족으로
   영영 안 채워지는 경로**에 들어가 있다. 후보:
   - **(a) 1건당 메시지 1개.** 모호함이 사라지고 종목별로 답할 수 있다. 대가는 알림
     개수 — 하루 6건이면 메시지 6개다. ⚠️ **알림 폭주는 10장이 이미 리스크로 세어 둔
     것**이고, 시끄러워서 안 읽기 시작하면 주의력 기계가 무너진다(1.2). 묶음 요약 1개 +
     개별 메시지 N개로 나누는 절충도 있다.
   - **(b) 종목별 버튼을 한 메시지에 N행으로.** `callback_data`에 `signal_id`가 실리므로
     기술적으로는 지금도 된다 — `ack_buttons`가 1건으로 좁힌 것은 **메시지당 답이 하나**라는
     가정 때문이었고, 그 가정을 버리면 6종목이면 버튼 12개짜리 메시지가 된다.
   - **(c) 그대로 둔다.** 오버라이드 추적을 포기하는 것과 같으므로, 고른다면 4.8에서
     그 항목을 내려야 한다 — **못 채울 숫자를 표에 남겨 두지 않는다.**
