...
```
📈 trend_breakout_55 — 신호 1건 [15:40 [krx]]

· 성호전자 (krx:043260)  15,870 (+6.22%)
   krx 1위/195 · trend_strength=3.409 · tsmom_12m=1.167 · stop_2n=14210
   marketscan explain 42

          [ ✅ 샀다 ]   [ ⬜ 안 샀다 ]
```

> `acted`는 `--commit`의 부작용 셋(알림·`signals` 기록·봉 소비)에 **들어가지 않습니다.**
> 규칙 11이 막는 것은 되돌릴 수 없는 것인데, 잘못 눌러도 반대로 다시 누르면 그만입니다.

**2. 봉이 안 끊기게 한다** (규칙 18) — 3의 전제조건

- [ ] `ingest`의 수집 대상에 `signals`의 종목 합집합을 더한다
- [ ] ⚠️ 유니버스에서 밀리는 종목은 **대개 내린 종목**이라, 봉이 끊기는 쪽도 손실 신호에
      몰립니다. **손실만 골라서 통계에서 빠지는 것**이라 3보다 먼저 해야 합니다

**3. 사후 수익률을 채운다** (`Forward Return Evaluator`)

- [ ] `signals`에 `fwd_1`·`fwd_5`·`fwd_20` 컬럼 + `as_of`로부터 N봉 뒤 종가를 캐시에서 찾는 평가기.
      **캐시가 붙어서 외부 호출이 필요 없습니다**
- [ ] 아직 N봉이 안 지난 신호는 **`NULL`로 두고 집계에서 뺀다** — 0으로 채우면 최근 신호가
      전부 "수익률 0%"로 잡혀 통계가 조용히 희석됩니다
- [ ] 봉 개수는 `MarketCalendar` 기준 (휴장일 때문에 "20일 뒤"와 "20봉 뒤"는 다릅니다)
- [ ] ⚠️ **DB 마이그레이션 경로** — 컬럼을 더하면 기존 DB가 `SchemaDriftError`로 막힙니다
      (의도된 방어). `ohlcv_cache`·`signals`는 자산이므로 지우기 전에 백업합니다

**4. 월간 성적표 → 텔레그램** ★ 제품

- [ ] `stats`에 forward return 분포(중앙값·IQR) · hit rate · **IC** (§4.8)
- [ ] ★ **오버라이드 비교** — 내가 산 것 vs 무시한 것의 사후 성적. `acted`와 3이 만나는 자리이고,
      **이 시스템에서 가장 확실한 가치가 여기서 나옵니다**
- [ ] **벤치마크 지수 수집** — KOSPI · S&P500. 상승장에선 아무거나 찍어도 hit rate가 65%라,
      빼고 보지 않으면 **시장이 좋았던 것을 전략의 공으로 돌립니다.** 지금은 안 모읍니다
- [ ] `serve`가 한 달에 한 번 보낸다 + **일일 알림에 "이 전략 최근 20건 성적" 한 줄**

**5. 엔진 검증 4종** — 위 숫자를 믿어도 되는지

- [ ] **난수 신호** — hit rate가 유니버스 기저율과 일치해야 합니다.
      **넘으면 전략 문제가 아니라 미래 참조가 있는 것입니다.** AST 검사가 못 잡는
      경로(조인 실수·인덱스 밀림·유니버스 look-ahead)를 **결과로** 잡는 유일한 방법입니다
- [ ] 전량 매수 · 신호 1일 밀기 · 상장폐지 포함
