...
> 1. 텔레그램 [샀다/안 샀다] 버튼             ← 오늘부터 응답이 쌓여야 한 달 뒤 성적표에 내용이 있다
...
**1. 텔레그램에서 바로 답한다** — `acted`를 실제로 채운다

- [ ] 알림에 인라인 버튼 `[샀다] [안 샀다]`. `serve`가 콜백을 받아 `signals.acted`에 쓴다
- [ ] ⚠️ **가장 싸고 가장 값이 큰 작업입니다.** 지금은 터미널에서 `signals ack 3 --ignored`를
      쳐야 해서 **아무도 안 하고**, 그래서 오버라이드 데이터가 영영 안 쌓입니다.
      **오늘부터 응답이 쌓여야 한 달 뒤 성적표에 내용이 있습니다** — 나중에 하면 그 한 달은
      영영 빈칸입니다
