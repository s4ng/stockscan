...
★ **파라미터는 이 파일이 정본입니다.** 설정 파일(`config.yml`)에는 전략 이름만
적고 값은 여기 아래 `Params`에 있습니다 — 값이 "왜 이 값인가"를 적은 이 docstring
바로 옆에 있어야, 설정에서 슬쩍 바꿔 돌려 보는 경로가 생기지 않습니다.
...
#: 거래일 기준 근사치. **표준 정의를 시장에 맞춰 바꾸지 않는다** — 한국과 미국의
#: 연간 거래일이 조금 다르지만, 정의를 흔들기 시작하면 그 순간부터 파라미터
#: 탐색이 된다 (4.8).
