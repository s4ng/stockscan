...
class ProviderRegistry:
    ...
    async def close(self) -> None:
        ...
        지금 등록된 소스는 전부 커넥션을 들고 있지 않지만, 계약은 남겨 둔다 —
        나중에 세션을 여는 소스가 붙었을 때 정리 지점을 다시 찾지 않아도 된다.
        """
...
#:
#: **모든 venue에 두 번째 소스를 둔다.** 무료 소스는 언제든 깨진다는 전제이고(3.9),
...
def default_registry() -> ProviderRegistry:
    ...
    소스 생성은 네트워크를 타지 않는다. pykrx/yfinance 임포트도 첫 조회 때로
    미뤄진다.
    """
