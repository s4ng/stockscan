"""성적표 — **이 프로젝트의 제품** (ARCHITECTURE.md 4.8).

일일 알림은 원료다. 모르는 종목의 순위를 받아도 할 수 있는 다음 행동이 없다.
제품은 **"내가 정한 규칙이 실제로 어땠는가"**이고, 그것이 여기서 나온다.
...
def render(card: Scorecard) -> str:
    """★ **이 메시지가 제품이다.** 한 달에 한 번 오는 이것을 읽으려고 나머지가 있다.
