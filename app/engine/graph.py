...
from app.market.timeframe import JUDGEMENT, is_judgeable
...
def validate(spec: PipelineSpec, mode: ExecutionMode | None = None) -> ValidationResult:
    ...
    for node in spec.nodes:
        ...
        if node.on_error.policy is ErrorPolicy.ROUTE and not _has_error_edge(spec, node.id):
            add(
                ValidationIssue(
                    IssueLevel.WARNING,
                    "on_error=route인데 error 핸들에 연결된 엣지가 없습니다 — "
                    "오류가 조용히 사라집니다",
                    node.id,
                )
            )
    ...
    for edge in spec.edges:
        ...
        if source is None:
            add(
                ValidationIssue(
                    IssueLevel.ERROR,
                    f"엣지의 source 노드가 없습니다: {edge.source}",
                    edge_id=edge.id,
                )
            )
        if target is None:
            add(
                ValidationIssue(
                    IssueLevel.ERROR,
                    f"엣지의 target 노드가 없습니다: {edge.target}",
                    edge_id=edge.id,
                )
            )
    ...
    # ---- 타임프레임 정책 -------------------------------------------------------
    result.issues.extend(_timeframe_issues(spec))

    return result


def _timeframe_issues(spec: PipelineSpec) -> list[ValidationIssue]:
    """판단 단위를 `1d`/`1w`로 제한한다 (ARCHITECTURE.md 3.6 / 규칙 12).

    v0.4는 이 게이트를 백테스트에만 걸었지만, v0.5는 **모든 모드**에 건다. 분봉을
    제외하는 이유가 "과거 이력 확보 비용"에서 "왕복 비용이 기대 수익을 넘는다"로
    바뀌었고, 후자는 실매매에도 그대로 적용되기 때문이다.

    ⚠️ 막는 곳은 **여기(정책 계층)뿐이다.** 타입과 캘린더의 분봉 분기는 건드리지
    않으므로, 나중에 여는 것은 이 함수를 고치는 일이지 재설계가 아니다.
    """
    ...
    for node in spec.nodes:
        ...
        if not isinstance(timeframe, str):
            continue
        try:
            allowed = is_judgeable(timeframe)
        except ValueError:
            continue  # 알 수 없는 타임프레임은 파라미터 검증이 이미 잡았다
        if not allowed:
            issues.append(
                ValidationIssue(
                    IssueLevel.ERROR,
                    f"판단 단위는 {', '.join(sorted(JUDGEMENT))}뿐입니다 (요청: {timeframe}). "
                    f"분봉 왕복 비용이 기대 수익을 넘고 그 구간의 상대는 호가창을 봅니다 — "
                    f"의도된 제약입니다. timeframe을 1d 또는 1w로 바꾸세요.",
                    node.id,
                )
            )
    return issues
