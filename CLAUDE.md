...
| **`scorecard`** | 성적표 — 신호를 전부 샀다고 치고 승률·기저율·초과수익 |
...
★ **성적표는 신호를 전부 샀다고 가정한다** (2026-08-07). 사용자가 실제로 무엇을 샀는지는
묻지 않는다. **가정을 리포트에 매번 적는다** — 안 적으면 이 숫자를 자기 계좌 수익률로
읽게 되고, 그 오독이 정확히 여기서 막으려는 자신감 기계다 (`tests/test_scorecard.py`가 고정).

⚠️ **`[샀다/안 샀다]` 응답을 되살리지 않는다** (2026-08-07에 걷어냄). 버튼은 신호 1건짜리
알림에만 붙일 수 있는데 실제 신호는 하루 3~6건이라 거의 안 붙었고, 무엇보다 **응답을
빠짐없이 해야만 성립하는 비교**였다 — 산 것만 답하고 무시한 것은 넘기면 acted/ignored가
**자기가 고른 분할**이 되어 결론이 아첨하는 쪽으로 기운다. **절반만 쓰면 없느니만 못하다.**
되살리려면 응답을 빠짐없이 받을 방법이 먼저 있어야 한다. `signals.acted` **컬럼은
남아 있다** — 자산 테이블이라 컬럼을 지울 수 없어서지, 쓰라는 뜻이 아니다.
...
기다린 뒤 매월 오는 성적표를 읽는다. 목록은 `ARCHITECTURE.md` §9 구현 로드맵을 본다
(2026-08-06에 README에서 옮겨 왔다 — README는 "무엇이고 어떻게 쓰는가"만 갖는다).
...
2. ★ **하트비트를 빼지 않는다.** 신호 0건이어도 하루 1회 보낸다. 없으면 "신호가 없는 것"과
   "프로세스가 죽은 것"이 구분되지 않는다. ⚠️ 단 **주말에는 발화도 하트비트도 쉰다**
   (2026-08-07, 설정이 아니라 기본값). **판정은 UTC 요일이다** — 로컬 요일로 자르면
   "토요일 06:10"(미국장 금요일 마감 직후)이 사라져 주중 하루치를 잃는다.
   성적표 날이 주말이면 다음 발송일로 민다(안 밀면 그 달치가 통째로 사라진다).
...
4. `run --commit`을 무엇이 부르든 동작이 같아야 한다 — 그 단일 출처가 **`app/service.py`**다.
   CLI도 스케줄러도 여기를 지난다.
5. ⚠️ **알림은 나가는 방향뿐이다.** `serve`가 텔레그램에서 무언가를 **받아 오는** 코드
   경로를 만들지 않는다 (`getUpdates`·콜백 수거). 2026-08-07에 그 방향을 통째로 닫았다.
...
| 주말에 하트비트가 안 옴 | 의도된 기본값이다. 판정은 UTC 요일이라 토요일 새벽 미국장 슬롯은 그대로 돈다 |
| `signals list`가 비어 있음 | `--commit` 없이 돌렸다. dry-run은 DB를 만들지도 않는다 |
| 알림이 안 오는데 로그엔 '보냄' | 채널이 `log`다 — 텔레그램 토큰 미설정. 기록만 하고 아무 데도 안 보낸다 |
| 성적표 수익률이 내 계좌와 다름 | 정상이다. 신호를 **전부 샀다고 가정**한 값이라 산 적 없는 종목도 들어 있다 |
