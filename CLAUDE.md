...
**현재 Phase 1·2 완료 — 다음은 Phase 3(리뷰 & 상주 실행)의 `3a. 데이터`부터다.**
목록은 `README.md`의 체크박스를 본다. 3a(사후 수익률·과거 신호 종목 수집·벤치마크 지수)
→ 3b(`review` + 차트) → 3c(`serve`) 순서이고, 앞이 안 서면 뒤가 못 선다.
...
코인·한국·미국을 동시에 훑는다.** 기본은 `pipelines/trend.yaml`(추세추종 55일 돌파)이고,
12-1 횡단면 모멘텀은 `pipelines/demo.yaml`에 남아 `-p`로 부른다.
...
| 캐시·신호가 갑자기 비어 보임 | 다른 디렉터리에서 실행해 DB가 새로 생겼는지 본다. `database_url()`이 프로젝트 루트로 펴지만 `MARKETSCAN_DATABASE_URL`을 절대경로로 덮었다면 그쪽이 이긴다 |
