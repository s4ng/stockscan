...
| **`serve`** | 웹 UI + 스케줄 + 알림 + **하루 1회 하트비트** (§8) |

**예측 기계가 아니라 주의력 기계다** — 시장을 맞히는 것이 아니라 혼자서는 볼 수 없는 범위를
대신 보고 정해둔 규칙을 대신 지킨다. 최종 판단은 사람이 한다.

★ **스크리너는 내버려 두면 자신감 기계가 된다** — 사람은 맞은 종목만 기억한다. 그걸 막는
장치는 지금 셋이다: `signals`에 **전부** 남는 기록, `backtest` 리포트의 붙박이 경고,
그리고 "백테스트를 대하는 자세"(아래). **셋 중 하나라도 무르게 만들지 않는다.**

⚠️ **`review`(신호 이력 + 이후 경로)는 없앴다.** `backtest`가 그 자리를 대신하지 **않는다** —
하나는 내가 고른 종목의 과거 재생이고 다른 하나는 실제로 낸 신호의 사후 성적이다. 후자를
보는 창구는 `signals`·`stats`인데 **사후 수익률·hit rate·IC가 아직 비어 있다**(Phase 3.5).
그만큼 out-of-sample 검증이 얇다는 것을 알고 쓴다.

**`ARCHITECTURE.md`가 설계의 단일 출처다.** 구조를 바꾸는 작업 전에 반드시 읽고,
설계를 바꿨다면 그 문서도 함께 갱신한다.

**현재 Phase 1·2·3 완료 — 다음은 Phase 3.5(사후 검증)다.**
목록은 `README.md`의 체크박스를 본다. `review`를 없애며 비워 둔 자리 — 신호별 사후
수익률 → `stats`의 hit rate·IC → 엔진 검증 4종 — 이 남아 있고, **그때까지
out-of-sample 검증이 얇다**는 것을 알고 쓴다.
...
| 자동 실행 | APScheduler | **`serve`** (§11-4b) — 상주 프로세스라 하트비트가 값이다 |

**시세 소스 4종은 전부 실물이고**(PyKRX · yfinance · FDR · CCXT), **파이프라인 하나가
코인·한국·미국을 동시에 훑는다.** 기본은 `~/.marketscan/config.yml`(추세추종 55일 돌파)이고,
12-1 횡단면 모멘텀은 `demo.yaml`에 남아 `-p`로 부른다.
유니버스 노드는 **하나**이고 `venues` 목록을
받는다 — 시장마다 노드를 만들면 `Bundle.merge`가 `context["universe"]`를 덮어써
두 시장이 소리 없이 사라진다.

**`serve`는 FastAPI + Jinja 서버 렌더 UI다** (2026-08-04 결정, v0.5 전환의 부분 번복).
걷어냈던 것은 *파이프라인을 캔버스로 편집하는 React 앱*이고, 되살린 것은 **이미 있는 명령을
부르고 결과를 띄우는 화면**이다. 그 차이가 유지돼야 한다:

- **Node·pnpm·TypeScript·빌드 단계는 여전히 없다.** 템플릿은 Jinja, 차트는 vendoring한
  lightweight-charts를 그대로 쓴다. `package.json`을 만들지 않는다.
- **화면은 엔진을 직접 부르지 않는다.** 실행은 전부 `app/service.py`를 지난다 —
  버튼과 CLI가 다른 일을 하기 시작하면 규칙 11·13이 화면 쪽으로 우회된다.
- `docker-compose.yml` · `Dockerfile` · `frontend/`는 그대로 되살리지 않는다.
...
반년 전 판단을 되짚는 화면인데 링크가 죽으면 존재 이유가 사라진다. 빌드 단계도
`package.json`도 만들지 않는다.
...
**`serve`가 완성됐다 — 웹 UI + 스케줄 + 알림 + 하트비트.** 지켜야 할 것 넷:

1. **`allow_alerts`를 켜는 자리는 `app/serve.py`의 스케줄 발화 하나뿐이다** (§12.2).
   화면 버튼도 터미널도 켜지 않는다 — 손으로 돌릴 때마다 채널로 나가면 알림을 믿지 않게 된다.
...
uv run marketscan serve             # 웹 UI + 스케줄 + 알림 (기본 127.0.0.1:8765)
...
- **전략은 그 설정 파일과 같은 디렉터리에서 찾는다.** 설정과 전략은 한 벌이라 통째로
  ...
  `-p sample/demo.yaml`은 `sample/`의 전략을 집는다.
...
18. **한 번이라도 신호가 난 종목은 수집을 멈추지 않는다** (§3.9). 유니버스에서 밀렸다고
    봉 수집이 끊기면 `backtest`의 차트가 그 시점에 멈추는데, **하필 밀린 종목이 대개 내린
    종목이라** 화면이 조용히 낙관 편향된다. `ingest` 대상에 `signals`의 종목을 더한다.
...
| 알림이 안 오는데 화면엔 '보냄' | 채널이 `log`다 — 텔레그램 토큰 미설정. 기록만 하고 아무 데도 안 보낸다 |
...
| `DB 스키마가 모델보다 낡았습니다` | 모델에 컬럼을 더했는데 `create_all`이 기존 테이블을 못 고친다. 백업 후 DB 파일을 지우면 재생성된다 |
